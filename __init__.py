...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    async def update_ip_service(call):
        entry_id = call.data.get("entry_id")
        ip_address = call.data.get("ip_address")
        if not entry_id or not ip_address:
            LOGGER.error("update_ip service requires entry_id and ip_address")
            return
            
        entry = hass.config_entries.async_get_entry(entry_id)
        if entry:
            LOGGER.info(f"Updating Elegoo Printer IP to {ip_address} for entry {entry_id}")
            new_data = {**entry.data, CONF_IP_ADDRESS: ip_address}
            new_options = {**entry.options, CONF_IP_ADDRESS: ip_address}
