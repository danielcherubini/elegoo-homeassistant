...
| **1883** | TCP/MQTT | MQTT broker | MQTT broker |
...
ElegooSlicer (based on OrcaSlicer) generates G-code files with a well-defined structure. The filament usage data at the end of the file is particularly relevant for tracking per-spool consumption with multi-material Canvas setups.

### File Layout

```
; HEADER_BLOCK_START
; generated by ElegooSlicer ...
; total layer number: N
; filament_density: d1,d2,d3,d4
; filament_diameter: 1.75,1.75,1.75,1.75
; max_z_height: H
; HEADER_BLOCK_END

; THUMBNAIL_BLOCK_START
; thumbnail begin 144x144 NNNN
; <base64 PNG data>
; thumbnail end
; THUMBNAIL_BLOCK_END

; extrusion widths, print parameters...

; EXECUTABLE_BLOCK_START
<actual G-code commands>
; EXECUTABLE_BLOCK_END

; filament used [mm] = v1, v2, v3, v4      ← per-slot millimeters
; filament used [cm3] = v1, v2, v3, v4     ← per-slot volume
; filament used [g] = v1, v2, v3, v4       ← per-slot weight (MOST USEFUL)
; total filament used [g] = N
; total filament cost = N
; total layers count = N
; estimated printing time (normal mode) = Xm Ys

; CONFIG_BLOCK_START
; <all slicer settings as key = value comments>
; CONFIG_BLOCK_END
```
...
>
> On stock firmware, G-code file content cannot be downloaded from the printer (see [Method 1057](#method-1057--download_file)). To access per-slot filament data, the file must be captured at upload time using a **local network proxy** between the slicer and the printer.
>
> **Proxy approach:** A local network proxy intercepts the slicer's `PUT /upload` request on port 80, saves a copy of the G-code file, and forwards the request to the printer. The slicer is pointed at the proxy's IP instead of the printer's IP. The proxy also provides TCP pass-through on ports 1883 (MQTT) and 8080 (camera) so the slicer's Device page continues to work normally. The HA integration connects directly to the printer and does not need to go through the proxy.
