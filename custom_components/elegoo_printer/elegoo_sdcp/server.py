...
class ElegooPrinterServer:
    ...
    def __init__(self, printer: Printer, logger: Logger):
        """
        ...
        if self.is_port_in_use(LOCALHOST, WEBSOCKET_PORT):
    ...
    def get_printer(self) -> Printer:
        ...
        proxied_printer.ip_address = LOCALHOST
    ...
    def _start_servers_in_thread(self):
        ...
        def ws_handler(ws):
            ...
            return self._proxy_handler(ws, self.printer.ip_address, self.logger)

        start_ws_server = serve(ws_handler, INADDR_ANY, WEBSOCKET_PORT)
        ...
        def discovery_protocol_factory():
            ...
            """
            return DiscoveryProtocol(self.logger, self.printer)

        start_discovery_server = loop.create_datagram_endpoint(
            discovery_protocol_factory, local_addr=(INADDR_ANY, DISCOVERY_PORT)
        )
    ...
    def is_port_in_use(self, host: str, port: int) -> bool:
        ...
        """
        self.logger.debug(f"Checking for port {port} on {host}")
    ...
    async def _proxy_handler(
        self, local_client_ws: WebSocketServerProtocol, remote_ip: str, logger: Logger
    ):
        """
        ...
        try:
            ...
            async with connect(remote_uri) as remote_printer_ws:
                ...
                forward_to_printer = asyncio.create_task(
                    self._forward_messages(local_client_ws, remote_printer_ws, logger)
                )
                forward_to_client = asyncio.create_task(
                    self._forward_messages(remote_printer_ws, local_client_ws, logger)
                )
    ...
    async def _forward_messages(
        self, source: Forwardable, dest: Forwardable, logger: Logger
    ):
        """
...
class DiscoveryProtocol(asyncio.DatagramProtocol):
    ...
    def __init__(self, logger: Any, printer: Printer):
        """
