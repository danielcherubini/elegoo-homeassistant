...
class ElegooMqttClient:
    ...
    async def ping_printer(self, ping_timeout: float = 5.0) -> bool:
        """
        Test if printer is reachable by checking MQTT port connectivity.
        ...
        try:
            _, writer = await asyncio.wait_for(
                asyncio.open_connection(self.printer.ip_address, MQTT_PORT),
            ...
            self.logger.debug(
                "Printer MQTT ping failed at %s:%s - %s",
            ...
            self.logger.debug(
                "Printer MQTT ping successful at %s:%s",
                self.printer.ip_address,
                MQTT_PORT,
            )
            return True
